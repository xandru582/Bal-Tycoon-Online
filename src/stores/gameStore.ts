...
export const useGameStore = create<GameState>((set, _get) => ({
  ...
  tick: (deltaSeconds) => set((state) => {
    ...
    const eff = deltaSeconds * state.gameSpeed;
    const earned = state.creditsPerSecond * eff;
    let nc = state.credits + earned;
    const nt = state.totalCreditsEarned + earned;

    // Economy engine tick (2 real seconds = 1 game day)
    state.engine.update(eff * 2);
    const newDay = state.engine.totalDays;
    const dayChanged = newDay > state.currentDay;

    // Stock market tick on day change
    if (dayChanged) {
      state.stockMarket.update(newDay, state.engine.indicators.phase);
    }

    // Contract tick on day change
    const newNotifs: Notification[] = [];
    let contractPay = 0;
    let contractPenalty = 0;
    if (dayChanged) {
      // FIRST: apply company dailyTick costs (salaries/maintenance) to nc.
      // company.cash was synced to state.credits at end of last tick, then dailyTick deducted costs.
      // Delta = company.cash - state.credits = -costs. Add delta to nc to preserve earned CPS.
      if (state.engine.playerCompany) {
        const costDelta = state.engine.playerCompany.cash - state.credits;
        nc += costDelta; // subtracts daily costs, preserves CPS earned
        if (nc < 0) {
          nc = 0;
          state.engine.playerCompany.cash = 0;
          newNotifs.push(makeNotification('danger', 'Sin Fondos', 'La empresa no puede pagar sus costes operativos este día.', '🚨'));
        }
      }

      // THEN: process dividend income
      const dividendIncome = state.stockMarket.getDividendIncome(newDay);
      if (dividendIncome > 0) {
        nc += dividendIncome;
        newNotifs.push(makeNotification('success', 'Dividendos', `+Đ${Math.floor(dividendIncome).toLocaleString()} en dividendos de tus acciones`, '💰'));
      }

      // THEN: process contract payments/penalties
      const inv: Record<string, number> = {};
      const company = state.engine.playerCompany;
      if (company) {
        company.inventory.forEach((v, k) => { inv[k] = v; });
      }
      const ctr = state.contractManager.tick(newDay, nc, inv);
      contractPay = ctr.payments;
      contractPenalty = ctr.penalties;
      nc += contractPay - contractPenalty;
      for (const evt of ctr.events) {
        const type = evt.startsWith('✔') ? 'success' : evt.startsWith('❌') || evt.startsWith('💥') ? 'danger' : 'info';
        newNotifs.push(makeNotification(type, 'Contratos', evt, '📋'));
      }
      if (contractPenalty > 0) {
        newNotifs.push(makeNotification('danger', 'Penalización', `Đ${Math.floor(contractPenalty).toLocaleString()} deducidos por contratos`, '⚠️'));
      }

      // Refresh available contracts
      const sector = state.engine.playerCompany?.sector ?? 'tecnologia';
      const rep = state.engine.playerCompany?.reputation[sector] ?? 30;
      state.contractManager.refreshAvailable(sector, newDay, rep);

      // Sync nc back to company.cash so dailyTick sees correct value next day
      if (state.engine.playerCompany) {
        state.engine.playerCompany.cash = nc;
      }

      // Rival manager tick
      const rivalEvents = state.rivalManager.update(newDay, state.engine.indicators.phase);
      for (const re of rivalEvents.slice(0, 2)) {
        newNotifs.push(makeNotification(re.type === 'attack' ? 'danger' : 'info', re.description.slice(0, 40), re.description, '🏢'));
      }

      // Personal Life Monthly Process (every 30 days, not just on day%30==1 to avoid skips at high speed)
      const lastMonthDay = Math.floor((state.currentDay) / 30);
      const newMonthDay = Math.floor(newDay / 30);
      if (newMonthDay > lastMonthDay && newDay > 1) {
        // Cap salary at 70% of current nc to avoid draining all company funds
        const cappedSalary = Math.min(state.personalManager.state.salary, nc * 0.7);
        const savedSalary = state.personalManager.state.salary;
        state.personalManager.state.salary = cappedSalary;
        const monthly = state.personalManager.processMonth(nc, newDay);
        state.personalManager.state.salary = savedSalary;
        nc -= monthly.netSalary;
        if (!monthly.success) {
          newNotifs.push(makeNotification('danger', 'Vida Personal', 'No has podido pagar el mantenimiento de tu estilo de vida. Tu estrés se dispara.', '🚨'));
        }
      }
    }

    // Personal Life Tick (Every frame)
    state.personalManager.tick(
      state.engine.playerCompany?.getFinancialSummary()?.revenue30d ?? 0,
      state.engine.playerCompany?.workers.length ?? 0,
      eff * 2,
      newDay
    );

    // Efficiency multiplier based on stress/happiness
    let efficiencyMultiplier = 1;
    if (state.personalManager.state.stress > 70) {
      efficiencyMultiplier = Math.max(0.4, 1 - (state.personalManager.state.stress - 70) * 0.015);
    } else if (state.personalManager.state.happiness > 80) {
      efficiencyMultiplier = 1.2;
    }

    // Apply efficiency to the passive earnings portion only
    const efficiencyPenalty = earned * (1 - efficiencyMultiplier);
    nc = Math.max(0, nc - efficiencyPenalty);

    // Completed contracts counter
    let completedDelta = 0;
    if (dayChanged) {
      completedDelta = Math.max(0, state.contractManager.history.length - state.completedContracts);
    }

    // Sync player company cash to match nc
    if (state.engine.playerCompany) {
      state.engine.playerCompany.cash = nc;
    }

    // Influence trickle
    const inf = state.influence + (dayChanged ? 1 : 0);

    // Achievement check on day change
    const base = { credits: nc, totalCreditsEarned: nt, currentDay: newDay, influence: inf, completedContracts: state.completedContracts + completedDelta };
    let achResult = { achievements: state.achievements, bonus: 0, toasts: [] as Notification[] };
    if (dayChanged) {
      achResult = checkAchievements({ ...state, ...base });
    }

    // Game events: check for new event on day change (only if no event is already pending)
    let newPendingEvent: GameEvent | null = state.pendingEvent;
    let newLastEventRealTime = state.lastEventRealTime;

    if (!state.pendingEvent) {
      const now = Date.now();
      const EVENT_INTERVAL_MS = 10 * 60 * 1000; // 10 real minutes

      // Timed forced event: every 10 real minutes regardless of game day
      if (now - state.lastEventRealTime >= EVENT_INTERVAL_MS) {
        const forced = state.gameEvents.forceRandomEvent(newDay);
        if (forced) {
          newPendingEvent = forced;
        }
        newLastEventRealTime = now;
      }
      // Also check probability-based events on day change (organic)
      else if (dayChanged) {
        const triggered = state.gameEvents.checkForEvents(newDay, {
          credits: nc,
          creditsPerSecond: state.creditsPerSecond,
          influence: inf,
          currentDay: newDay,
          personalManagerStress: state.personalManager.state.stress,
          personalManagerHappiness: state.personalManager.state.happiness,
        });
        if (triggered.length > 0) {
          newPendingEvent = triggered[0];
        }
      }
    }

    // Apply active event CPS multiplier
    const eventCpsMult = state.gameEvents.getCombinedCpsMultiplier();

    return {
      ...base,
      _rev: dayChanged ? state._rev + 1 : state._rev,
      credits: nc + achResult.bonus,
      totalCreditsEarned: nt + achResult.bonus,
      achievements: achResult.achievements,
      notifications: [...state.notifications.slice(-50), ...newNotifs],
      toastQueue: [...state.toastQueue, ...achResult.toasts],
      pendingEvent: newPendingEvent,
      lastEventRealTime: newLastEventRealTime,
      creditsPerSecond: state.upgrades.reduce((a, u) => a + u.purchased * u.cpsEach, 0) * eventCpsMult * (state.engine.playerCompany?.getLevelCpsMultiplier() ?? 1),
    };
  }),
